...
> Volcengine Ark / Doubao plugin bundle for Hermes Agent，支持 Agent Plan、Coding Plan、普通火山 Ark API，并提供文本模型、图像生成、视频生成和搜索 backend。

中文 README 可写为：

> 面向 Hermes Agent 的火山方舟 / 豆包插件集合，兼容 Agent Plan、Coding Plan 与普通 Ark 按量付费 API，提供模型 provider、图像生成、视频生成和豆包搜索 backend。
...
- 不在 `main` 上重构。
- 当前重构分支建议：`refactor/volcengine-plan-compat`
- 每个阶段独立提交。
